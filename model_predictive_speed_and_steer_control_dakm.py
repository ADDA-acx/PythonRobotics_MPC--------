...
def get_linear_model_matrix(v, phi, delta):
    ...
    A[3, 2] = DT * 2*math.tan(delta) / WB

    B = np.zeros((NX, NU))
    B[2, 0] = DT
    B[3, 1] = DT * 2*v / (WB * math.cos(delta) ** 2)

    C = np.zeros(NX)
    C[0] = DT * v * math.sin(phi) * phi
    C[1] = - DT * v * math.cos(phi) * phi
    C[3] = - DT * 2*v * delta / (WB * math.cos(delta) ** 2)
...
def plot_car(x, y, yaw, steer=0.0, cabcolor="-r", truckcolor="-k"):  # pragma: no cover
    ...
    Rot2_reverse = np.array([[math.cos(-steer), math.sin(-steer)],
                             [-math.sin(-steer), math.cos(-steer)]])

    fr_wheel = (fr_wheel.T.dot(Rot2)).T
    fl_wheel = (fl_wheel.T.dot(Rot2)).T
    rr_wheel = (rr_wheel.T.dot(Rot2_reverse)).T
    rl_wheel = (rl_wheel.T.dot(Rot2_reverse)).T
...
def update_state(state, a, delta):
    ...
    state.yaw = state.yaw + 2*state.v / WB * math.tan(delta) * DT
